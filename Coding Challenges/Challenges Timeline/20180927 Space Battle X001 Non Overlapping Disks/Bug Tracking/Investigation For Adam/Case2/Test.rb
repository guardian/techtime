...
class Challenge20180927
    ...
    def self.diskDoesNotIntersectMapNonTrivially(map, disk)
        ...
        #point = points.select{|point| MapUtils::distanceBetweenTwoMapPoints(point, disk["point"]).round(12) < disk["radius"].round(12) }.first
        #if point then
        #    puts JSON.pretty_generate([disk, point, MapUtils::distanceBetweenTwoMapPoints(point, disk["point"])])
        #end
        points.all?{|point| MapUtils::distanceBetweenTwoMapPoints(point, disk["point"]).round(12) >= disk["radius"].round(12) }
    end

    # Challenge20180927::individualDisksAreValid(map, disks)
    def self.individualDisksAreValid(map, disks)
        #puts disks.select{|disk| !Challenge20180927::diskDoesNotIntersectMapNonTrivially(map, disk) }.first
